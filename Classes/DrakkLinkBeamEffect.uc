class DrakkLinkBeamEffect extends LinkBeamEffect;
/*
...
simulated function Tick(float dt)
{
	...
	if ( Level.NetMode != NM_DedicatedServer )
	{
		...
		{
			if ( MuzFlash == None )
				MuzFlash = Spawn(class'DrakkLinkMuzFlashBeam3rd', self);
		}
		...
		if ( Sparks == None && EffectIsRelevant(EndEffect, false) )
		{
			...
			if ( (P == Instigator.Controller) || CheckMaxEffectDistance(P, Location) )
				Sparks = Spawn(class'DrakkLinkSparks', self);
		}
	}
    ...
    if ( Links != OldLinks || LinkColor != OldLinkColor || MuzFlash != OldMuzFlash )
    {
		...
		if ( Level.NetMode != NM_DedicatedServer )
		{
			for (c=0; c<MAX_CHILDREN; c++)
			{
				...
				{
					if ( Child[c] == None )
						Child[c] = Spawn(class'DrakkLinkBeamChild', self);
